...
initialize_globals (const string& name)
{
  ...
  Vuser_name = entry ? entry->pw_name : "unknown";

  char hostname[256];
  int status = gethostname (hostname, 255);
  Vhost_name = (status < 0) ? "unknown" : hostname;

  char *hd = getenv ("HOME");
  Vhome_directory = hd ? hd : "/";
...
initialize_pathsearch (void)
{
  ...
  char *odb = getenv ("OCTAVE_DB_PATH");

  // For backward compatibility.

  if (! odb)
    odb = getenv ("OCTAVE_DB_DIR");
  ...
  if (! odb)
    ...
    oct_putenv ("TEXMFDBS", odb);

  dir_path::init_program_name (program_invocation_name);
}
