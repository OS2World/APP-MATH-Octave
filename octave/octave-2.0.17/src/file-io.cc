      ...
      if (os)
	{
	  if (args(0).is_string ())
	      ...
	      if (nargin > 1)
		{
	      ...
	      retval(2) = (double) os.printf (fmt, tmp_args);
	      retval(1) = os.error ();
	      char *tmp = ostr.str ();
	      retval(0) = tmp;
	      delete [] tmp;
  ...
  COUNT     : number of elements written")
{
  ...
  if (nargin > 1 && nargin < 6)
      ...
      if (os)
	{
	  ...
	  retval = (double) do_fwrite (*os, data, prec, skip, arch);
	}
      else
...
symbols_of_file_io (void)
{
  ...
  DEFCONSTX ("stdin", SBV_stdin, 0.0,
    "file number of the standard input stream");

  DEFCONSTX ("stdout", SBV_stdout, 1.0,
    "file number of the standard output stream");

  DEFCONSTX ("stderr", SBV_stderr, 2.0,
