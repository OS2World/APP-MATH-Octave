...
command_history::command_history (const string& f, int n)
{
  if (initialized)
      ...
      if (! f.empty ())
	{
	  ...
	  char *FIX_READLINE_PROTOS = ::strdup (f.c_str ());

	  ::read_history (FIX_READLINE_PROTOS);
  ...
  if (! f.empty ())
    {
      ...
      int status = ::read_history (FIX_READLINE_PROTOS);
  ...
  if (! f.empty ())
    {
      ...
      int status = ::read_history_range (FIX_READLINE_PROTOS, from, to);
  ...
  if (! f.empty ())
    {
      ...
      int status = ::write_history (FIX_READLINE_PROTOS);
      ...
      if (lines_this_session < where ())
	{
	  ...
	  if (! f.empty ())
	      ...
	      if (! fs)
		{
	      ...
	      int status
		= ::append_history (lines_this_session, FIX_READLINE_PROTOS);
  ...
  if (! f.empty ())
    {
      ...
      ::history_truncate_file (FIX_READLINE_PROTOS, n);
...
{
  char *FIX_READLINE_PROTOS = ::strdup (line.c_str ());      

  HIST_ENTRY *discard
    = ::replace_history_entry (which, FIX_READLINE_PROTOS, 0);
...
command_history::clean_up_and_save (const string& f_arg, int n)
{
  ...
  if (! f.empty ())
	...
	n = xsize;
      ...
      ::write_history (FIX_READLINE_PROTOS);
