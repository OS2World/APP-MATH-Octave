...
_IO_FILE *
DEFUN(_IO_proc_open, (fp, command, mode, p_mode),
      ...
      AND int p_mode)
{
  ...
  child_std_end = (mode[0] == 'r' ? 1 : 0);
  org = _dup (child_std_end);
  if (org != -1)
    {
      ...
      child_pid = _IO_system (command, p_mode);
...
_IO_FILE *
DEFUN(_IO_popen, (command, mode, p_mode),
      const char *command AND const char *mode AND int p_mode)
{
...
#endif
  if (_IO_proc_open (fp, command, mode, p_mode) != NULL)
    return fp;
  free (fpx);
  return NULL;
}
      ...
      _IO_FILE *fp)
{
...
#if _IO_HAVE_SYS_WAIT
  volatile void *sigint_handler, *sigbreak_handler,  *sigchld_handler;
  ...
  sigbreak_handler = signal (SIGBREAK, SIG_IGN);
  sigchld_handler  = signal (SIGCHLD,  SIG_IGN);
