...
double merlin_j0(double x) 
{
  double z, s,c,ss,cc,r,u,v;

  if (!finite(x))  return 1.0/(x*x);
  ...
  if (x < 1.220703125e-004)
    {		   /* |x| < 2**-13 */
      if (_MERLIN_MATH_BIG+x > 1.0)
...
double merlin_y0(double x)
{
  double z, s,c,ss,cc,u,v,merlin_j0();

  if (!finite(x)) return (1.0/(x+x*x));
  ...
  if (x <= 7.450580596923828125e-009)
    {		/* x < 2**-27 */
  ...
  return (u/v + tpi*(merlin_j0(x)*log(x)));
}
