...
double merlin_j1(double x)
{
  ...
  if (y < 7.450580596923828125e-009)
    {	/* |x|<2**-27 */
      if(_MERLIN_MATH_BIG+x>1.0) return 0.5*x;    /* inexact if x!=0 necessary */
    }
  z = x*x;
...
double merlin_y1(double x)
{
  double z, s,c,ss,cc,u,v,merlin_j1();
  ...
  if (x >= 2)			 /* |x| >= 2.0 */
      ...
      if (x < .5 * DBL_MAX)	/* make sure x+x not overflow */
	{
      ...
      */
      ...
      if (x>two_129) z = (invsqrtpi*ss)/sqrt(x);
      else
	{
	  u = pone(x); v = qone(x);
  ...
  return (x*(u/v) + tpi*(merlin_j1(x)*log(x)-1.0/x));
}
