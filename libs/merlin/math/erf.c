...
**
**      6. For inf > x >= 28
**         	erf(x)  = 1 - ERF_tiny  (raise inexact)
**         	erfc(x) = ERF_tiny*ERF_tiny (raise underflow)
**
**      7. Special cases:
...
static double ERF_tiny=1e-300, ERF_c=8.45062911510467529297e-01;
  ...
  if (ax < .84375)
    {
      if (ax < 3.7e-09)
	{
	  if (ax < 1.0e-308)  return 0.125*(8.0*x+p0t8*x);
	  return x + p0*x;
	}
      y = x*x;
  ...
  if (ax < 1.25)
    {
      ...
      if (x>=0)  return (ERF_c + P/Q);
      else       return (-ERF_c - P/Q);
    }
  if (ax >= 6.0)
    {
      if (x >= 0.0)  return (1.0-ERF_tiny);
      else           return (ERF_tiny-1.0);
    }
  z = -ax*ax;
  s = -1.0/z;
  if (ax < 2.0)
...
double merlin_erfc(double x)
{
  ...
  if (ax < .84375)
    {
      if (ax < 1.38777878078144568e-17)  return 1.0-x;
      y = x*x;
      r = y*(p1+y*(p2+y*(p3+y*(p4+y*(p5+y*(p6+y*(p7+y*(p8+y*(p9+y*p10)))))))));
      if (ax < .0625)  return (1.0-(x+x*(p0+r)));
      else
	{
	  r = x*(p0+r);
	  r += (x-0.5);
  ...
  if (ax < 1.25)
    {
      ...
      P = pa0+s*(pa1+s*(pa2+s*(pa3+s*(pa4+s*(pa5+s*pa6)))));
      Q = 1.0+s*(qa1+s*(qa2+s*(qa3+s*(qa4+s*(qa5+s*qa6)))));
      if (x>=0)  {  z = 1.0-ERF_c; return z - P/Q;  }
      else       {  z = ERF_c+P/Q; return 1.0+z;    }
    }
  if (ax >= 28)
    {
      if (x>0)  return (ERF_tiny*ERF_tiny);
      else      return (2.0-ERF_tiny);
    }
  z = ax;
  TRUNC(z);
  y = z - ax;
  ...
  r = ((double) _expl(((long double) s)+((long double) y)))/x;
  if (x>0)  return r;
  else      return 2.0-r;
}
